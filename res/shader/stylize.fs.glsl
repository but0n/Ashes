...
void main() {
    ...
    color += lightContrib(vec3(1, 1, 5), core) * vec3(1.0, 0.7922, 0.4078) * 6.0;
    color += lightContrib(vec3(-5, 3, -5), core) * vec3(0.6431, 0.9176, 1.0);
    // color += lightContrib(vec3(-5, 1, 5), core) * vec3(0.3515625, 0.796875, 0.95703125) * 10.;
    // color += lightContrib(vec3(5, 1, -5), core) * vec3(0.921875, 0.4375, 0.9609375) * 10.;
